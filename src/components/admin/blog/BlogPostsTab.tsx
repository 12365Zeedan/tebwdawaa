          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border bg-muted/30">
                <th className="text-start px-6 py-3 text-sm font-medium text-muted-foreground">{isAr ? 'المقال' : 'Post'}</th>
                <th className="text-start px-6 py-3 text-sm font-medium text-muted-foreground">{isAr ? 'الكاتب' : 'Author'}</th>
                ...
                <th className="text-start px-6 py-3 text-sm font-medium text-muted-foreground">
                  <div className="flex items-center gap-1"><BarChart3 className="h-3.5 w-3.5" />{isAr ? 'المشاهدات' : 'Views'}</div>
                </th>
                ...
                posts.map((post) => {
                  ...
                  return (
                    <tr key={post.id} className="border-b border-border last:border-0 hover:bg-muted/20">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">{post.read_time} {isAr ? 'دقائق' : 'min read'}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-sm text-foreground">{author || '—'}</td>
                      <td className="px-6 py-4">{category ? <Badge variant="secondary">{category}</Badge> : '—'}</td>
                      <td className="px-6 py-4">
                        {post.is_published ? (
                      ...
                      <td className="px-6 py-4"><span className="text-sm font-medium text-foreground">{postViews.toLocaleString()}</span></td>
                      <td className="px-6 py-4">
