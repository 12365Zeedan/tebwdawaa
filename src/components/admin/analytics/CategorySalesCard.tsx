...
import type { TimePeriod, SortDirection } from '@/hooks/useSalesAnalytics';
import {
  Table,
  TableBody,
  TableCell,
...
export function CategorySalesCard() {
  ...
  const { data, isLoading } = useCategorySales(period);
  ...
  const sorted = useMemo(() => {
    ...
    return [...data].sort((a, b) => {
      ...
      switch (sortField) {
        case 'name':
          cmp = (language === 'ar' ? a.categoryNameAr : a.categoryName).localeCompare(
            language === 'ar' ? b.categoryNameAr : b.categoryName
          );
          break;
        case 'quantity':
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-foreground">
              {language === 'ar' ? 'المبيعات حسب التصنيف' : 'Sales Per Category'}
            </h2>
          </div>
                ...
                {sorted.map((row) => (
                  <TableRow key={row.categoryId}>
                    <TableCell className="font-medium">
                      {language === 'ar' ? row.categoryNameAr : row.categoryName}
                    </TableCell>
